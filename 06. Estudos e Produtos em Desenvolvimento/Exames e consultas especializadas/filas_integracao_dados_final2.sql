	...
	especialidades as (
		...
		tabela_detalhada as (
			...
			from 
				...
				join tipo_atendimento_agenda taa on oferta.cd_tipo = taa.cd_tipo
			where taa.cd_tipo in (286279540,286279601)
			order by 1
			)
select 
	especialidade,
	fila_total,
	prim_consulta_retorno_passado,
	retorno_futuro,
	solicit_ult_6m,
	devolvidos,
	case 
		when especialidade in ('GRUPO - EXAMES LABORATORIAIS',
								   'ELETROCARDIOGRAMA - ADULTO',
								   'GRUPO - RADIODIAGNÓSTICO ADULTO',
								   'GRUPO - RADIODIAGNÓSTICO PEDIATRIA', 
								   'GRUPO - ANATOMOPATOLOGIA E CITOPATOLOGIA',
								   'GRUPO - LAB ACESSO RÁPIDO',
								   'GRUPO - ULTRASSONOGRAFIA ADULTO', 
								   'MAMOGRAFIA BILATERAL PARA RASTREAMENTO',
								   'CONSULTA EM OFTALMOLOGIA ADULTO'
								   )
		then sum(vagas_ult_6m)
		else sum(case when ds_tipo = 'Fila Regulada' then vagas_ult_6m else 0 end)
		end as vagas_ult_6m
from 
	tabela_detalhada
