...
'O240','O241','O242','O243','P702')
	...
	exame_hbc as (
		...
		select 
			...
			dia.inicio_quad,
			dia.fim_quad,
			count(distinct(dia.cd_usu_cadsus)) as diabeticos,
			coalesce(sum(eh.fez_hbc),0) as hbc_6m,
			coalesce(sum(dia.consulta_6m),0) as consulta_6m,
			sum(case when dia.consulta_6m = 1 and eh.fez_hbc = 1 then 1 else 0 end) as hbc_e_consulta_6m
		from 
			diabeticos dia
