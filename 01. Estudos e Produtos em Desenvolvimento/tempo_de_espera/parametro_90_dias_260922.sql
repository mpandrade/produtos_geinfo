with 
	datas as (
		select generate_series('2019-01-01'::date,date_trunc('month',current_date + interval '1 month'),interval '1 month')::date as dt_referencia
	),
	solicitacoes as (
	select
        sa.cd_solicitacao,
	...
	where
		...
		and sa.status in (1,2) -- na fila, agendado
...
select 
	 nome_grupo,
	 d.dt_referencia as mes_ano,
	 sum(
	 	case 
			when inclusao > dt_referencia then 0
			when (agendamento between (dt_referencia - interval '1 month') and dt_referencia) then
				(case when (agendamento - inclusao) between 0 and 90 then 1 else 0 end)			
			when agendamento >= dt_referencia then 
				(case when (dt_referencia - inclusao) between 0 and 90 then 1 else 0 end)	
			when (agendamento is null and status = 1) then 
	 			(case when (dt_referencia - inclusao) between 0 and 90 then 1 else 0 end)
	 	else 0
	 ...
	 sum(
	 	case 
			when inclusao > dt_referencia then 0
			when (agendamento between (dt_referencia - interval '1 month') and dt_referencia) then
				(case when (agendamento - inclusao) > 90 then 1 else 0 end)			
			when agendamento >= dt_referencia then 
				(case when (dt_referencia - inclusao) > 90 then 1 else 0 end)	
			when (agendamento is null and status = 1) then 
	 			(case when (dt_referencia - inclusao) > 90 then 1 else 0 end)
	 	else 0
