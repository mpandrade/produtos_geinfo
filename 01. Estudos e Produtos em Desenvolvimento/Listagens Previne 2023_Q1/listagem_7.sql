with
	...
	diabeticos as (
		select 
			uc.cd_usu_cadsus,
			uc.dt_nascimento,
			coalesce(em.descricao, em2.descricao,'SEM UNIDADE REFERENCIA') as unidade,
			coalesce(eqa.cd_area::text, eqa2.cd_area::text,'SEM EQUIPE REFERENCIA') as equipe,
			q.inicio_quad,
			q.fim_quad,
			q.quad_texto,
				max(case
...
'O240','O241','O242','O243','P702')
	...
	exame_hbc as (
		...
		select
			...
			dia.inicio_quad,
			dia.fim_quad,
			dia.cd_usu_cadsus as diabetico,
			uc.dt_nascimento,
			coalesce(eh.fez_hbc,0) as hbc_6m,
			coalesce(dia.consulta_6m,0) as consulta_6m
		from 
			diabeticos dia
			left join exame_hbc eh on dia.cd_usu_cadsus = eh.cd_usu_cadsus
			join usuario_cadsus uc on uc.cd_usu_cadsus = dia.cd_usu_cadsus
