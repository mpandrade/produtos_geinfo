with
	...
	mulheres_25_64 as (
		...
		mulheres_c_cp as (
				...
				select
					coalesce(unidade,'SEM UNIDADE')as unidade_referencia,
					coalesce(equipe,'SEM EQUIPE')as equipe_referencia,
					fem.quad_texto,
					'indicador 4' as indicador,
					fem.inicio_quad,
					fem.fim_quad,
					mcp.cd_usu_cadsus,
					uc.dt_nascimento,
					mcp.fez_cp,
					de.denom_estim as denominador_estimado
				from
					mulheres_25_64 fem
				 	left join mulheres_c_cp mcp on fem.cd_usu_cadsus = mcp.cd_usu_cadsus
					left join denominador_estimado de on fem.quad_texto = de.quad_texto
					join usuario_cadsus uc on uc.cd_usu_cadsus = mcp.cd_usu_cadsus
				order by 1
